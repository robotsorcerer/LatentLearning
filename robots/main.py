__comment__     = "Solves the (stabilization) problem for various agents."
__author__ 		= "Lekan Molu and others"
...
class LatentLearner(object):
    def __init__(self, config):
        ...
        self.controller_type = FLAGS.controller_type
    ...
    def run(self, itr_start=0):
        try:
            if strcmp(self.controller_type,'analytic'):
                ...
                for itr in range(self._hyperparams['iterations']):
                    logger.info(f"Running GMR Controller on iteration {itr}/{self._hyperparams['iterations']}")
                    for cond in range(self._conditions):
