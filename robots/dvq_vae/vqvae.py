__all__ = [
            "VQVAE", "cos_anneal", "DecayTemperature", \
            "RampBeta", "DecayLR"
            ]
...
class VQVAE(pl.LightningModule):
    ...
    def validation_step(self, x, batch_idx):
        ...
        perplexity = ((-(avg_probs * torch.log(avg_probs + 1e-10)).sum()).exp()).double()
        cluster_use = torch.sum(avg_probs > 0).double()
    ...
    def configure_optimizers(self):
        ...
        # optimizer = torch.optim.AdamW(optim_groups, lr=1e-1, betas=(0.9, 0.999), eps=1e-08, weight_decay=1e-4)
        #We use the ADAM optimiser [21] with learning rate 2e-4
        optimizer = torch.optim.Adam(optim_groups, lr=1e-1, betas=(0.9, 0.999), eps=1e-08, weight_decay=1e-4)
    ...
    def add_model_specific_args(parent_parser):
        ...
        parser.add_argument("--enc_dec_flavor", type=str, default='openai', choices=['deepmind', 'openai'])
