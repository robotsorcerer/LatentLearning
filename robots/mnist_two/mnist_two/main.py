...
parser.add_argument('--data', default='maze', type=str, choices=['mnist', 'maze'])
...
parser.add_argument('--verbose', action='store_true', help='print dumps on screen?')
...
if args.data == 'mnist':
    ...
    raise Exception("Must be called with the --data [mnist|maze] argument.")
...
def init_model():
    net = Classifier(args, ncodes=ncodes, maxk=genik_maxk, inp_size=myenv.inp_size*2, verbose=args.verbose)
    if torch.cuda.is_available():
        net = net.cuda()
    return net
...
def main():
    ...
    for env_iteration in range(0, 200000):
        ...
        #do one step in env.
        if step == ep_length:
            print('reinit env') if args.verbose else None
            is_initial=True
            step = 0
        ...
        if args.data == 'mnist':
            ...
            raise Exception("<main.py> be called with the --data [mnist|maze] argument.")
        ...
        if always_random or mybuffer.num_ex < num_rand or step >= ep_rand or random.uniform(0,1) < 0.1:
            print('random action') if args.verbose else None
            a1 = myenv.random_action()
        else:
            print('use policy to pick action!') if args.verbose else None
            reward = transition.select_goal()
            print('init state abstract', init_state) if args.verbose else None
        ...
        print('my buffer numex', mybuffer.num_ex) if args.verbose else None
        ...
        for iteration in range(0,num_iter):
            ...
            if iteration % 100 == 0:
                print('loss', loss) #if args.verbose else None
