...
from visgrid.noise_processes import Ising
#from .sensors import *
...
class GridWorld(grid.BaseGrid):
    def __init__(self, noise_type, ising_beta, *args, **kwargs):
        ...
        if self.noise_type == 'ising':
            self.ising = Ising(N=80, beta=ising_beta)
            self.ising_noise = np.stack([(self.ising.reset()+1)/2]*3).transpose(1,2,0) * np.array([79,17,57])
        if self.noise_type == 'ellipse':
            self.config = {
                    "num_circles": 4,
                    "circle_width": 8,
    ...
    #     img = self.generate_noised_image(obs)
    #     return img        
    ...
    def get_obs(self, ax=None, noise_type=None, change_noise=False):
        if ax is None:
            ax = self.plot()
        ...
        if self.noise_type is not None:
            if change_noise:
                ...
                elif self.noise_type == 'ising':
                    self.ising_noise = np.stack([(self.ising.step()+1)/2]*3).transpose(1,2,0) * np.array([79,17,57])
            if self.noise_type == 'ellipse':
                X_noise = self.add_circles(X_noise)
            elif self.noise_type == 'ising':
                if self.ising.step > 20:
                    self.ising_noise = self.ising.reset()
                mask = (X_noise == 255)
                X_noise = (mask * (self.ising_noise) + (1-mask) * X_noise).astype(np.uint8)
            else:
                raise Exception('Unknown noise type')
        return X, X_noise
    
    def add_circles(self, img):#, exo_noise):
        width, height = img.shape[0], img.shape[1]
        image = Image.new('RGB', (width, height))
        draw = ImageDraw.Draw(image)
    ...
    def img(self):
        ...
        model_[self.goal_position[0], self.goal_position[1]] = 8

        return model_


    def get_observation(self, x, obs_type):
        if obs_type == 'high_dim':
    ...
    def transition(self,a1,a2): 
        ...
        next_state1, reward1, _ = self.step(a1)
        next_state2, reward2, _ = self.step(a2)

        return next_state1, next_state2


    def states_pos_to_int(self, test_s0, test_s1, rows, cols):
