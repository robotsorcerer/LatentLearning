...
from envs.noise_processes import Ising
...
class IsingWrapper(gym.Wrapper):
    def __init__(self, env, target_color=[170,1,20], ising_color=[202, 44, 146], beta=4., n_step=1):
        """
        Adds Ising noise to pixels of color `color`
        """
        self.env = env
        self.observation_space = env.observation_space
        self.action_space = env.action_space
        self.n_step = n_step
        self.N = min(self.observation_space.shape[:2])
        self.n_channels = self.observation_space.shape[-1]
        self.target_color = np.array(target_color)
        self.ising_color = np.array(ising_color)
        self.ising = [Ising(beta=beta,N=self.N) for _ in range(self.n_channels)]
        self.ising_noise = self._reset_ising()

    def _reset_ising(self):
        acc = []
        for i in range(self.n_channels):
            acc.append(self.ising[i].reset())
        return np.stack(acc).transpose(1,2,0)

    def _update_ising(self):
        acc = []
        for i in range(self.n_channels):
            acc.append(self.ising[i].step(n_steps=self.n_step))
        return np.stack(acc).transpose(1,2,0)

    def _add_noise(self,frame):
        mask = np.where( frame[:self.N,:self.N,:]==self.target_color )
        subframe = frame[:self.N,:self.N,:].copy()
        subframe[mask[0],mask[1]] = self.ising_color*(1+self.ising_noise[mask[0],mask[1]])/2
        frame[:self.N,:self.N,:] = subframe    
        return frame
    ...
    def step(self, action):
        ...
        self.ising_noise = self._update_ising()
...
class PixelRenderer():
    def __init__(self, env):
        ...
        self.observation_space = gym.spaces.Box(
            low=0,
            high=255,
            shape=(dummy_obs.shape[0],dummy_obs.shape[1], 3),
            dtype=np.uint8,
        )
