...
class Logger(object):
      ...
      def record_mean_rewards(self, mean_rewards):
            self.mean_rewards = mean_rewards

      def record_returns(self, train_returns):
            self.train_returns = train_returns

      def record_losses(self, train_losses):
            self.train_losses = train_losses

      def record_eval_return(self, eval_returns):
            self.all_mean_eval_rewards = eval_returns

      def record_eval_success(self, eval_success):
            self.all_eval_success = eval_success
      ...
      def save_agent_train_metric(self):
            np.save(os.path.join(self.save_folder, "mean_rewards_smooth.npy"), self.mean_rewards)
            np.save(os.path.join(self.save_folder, "train_returns.npy"), self.train_returns)
            np.save(os.path.join(self.save_folder, "train_losses.npy"), self.train_losses)
            np.save(os.path.join(self.save_folder, "eval_returns.npy"), self.all_mean_eval_rewards)
            np.save(os.path.join(self.save_folder, "eval_success.npy"), self.all_eval_success)
...
def plot_state_visitation(x, y, save_folder, bins):
      ...
      plt.imshow(H.T,origin='lower',extent=myextent,interpolation='nearest',aspect='auto')
      plt.plot(x,y,'ro')
      plt.colorbar()
