...
class DQNAgent():
    ...
    def reset(self):
        ...
        self.q = self.make_qnet(n_features=self.n_features,
                                ...
                                use_goal_conditioned=self.use_goal_conditioned,
    ...
    def act(self, x):
        if (len(self.replay) < self.n_steps_init or np.random.uniform() < self.get_epsilon()):
            ...
            with torch.no_grad():
                z_feature = self.phi(torch.tensor(x, dtype=torch.float32))
                if self.vq_layer is not None:
                    z, _, z_q_codebooks = self.vq_layer(z_feature)
                else:
                    z = z_feature
                q_values = self.q(z_feature, z)
    ...
    def v(self, x):
        with torch.no_grad():
            z_feature = self.phi(torch.stack(tch(x, dtype=torch.float32)))
            if self.vq_layer is not None:
                z, _, z_q_codebooks = self.vq_layer(z_feature)
            else:
                z = z_feature
            v = self.q(z_feature, z).numpy().max(axis=-1)
        return v

    def get_q_targets(self, batch):
        with torch.no_grad():
            zp_feature = self.phi(torch.stack(tch(batch.xp, dtype=torch.float32)))
            if self.vq_layer is not None:
                zp, _, z_q_codebooks = self.vq_layer(zp_feature)
            else:
                zp = zp_feature
            # Compute Double-Q targets
            ap = torch.argmax(self.q(zp_feature, zp), dim=-1)
            vp = self.q_target(zp_feature, zp).gather(-1, ap.unsqueeze(-1)).squeeze(-1)
    ...
    def get_q_predictions(self, batch):
        z_feature = self.phi(torch.stack(tch(batch.x, dtype=torch.float32)))
        if self.vq_layer is not None:
            z, _, z_q_codebooks = self.vq_layer(z_feature)
        else:
            z = z_feature
