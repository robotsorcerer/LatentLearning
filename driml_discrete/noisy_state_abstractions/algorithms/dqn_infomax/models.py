...
class VQ_infoNCE_Mnih_84x84_action_FILM(nn.Module): # input is (84,84,4)
    def __init__(self, obs_space, num_outputs, options):
        """
        3 Conv layers with 1 FC
        IM encoder is inserted after 3rd conv, and has 1x1 kernel
        ...
        super(VQ_infoNCE_Mnih_84x84_action_FILM, self).__init__()
        (w, h, in_channels) = obs_space.shape
        ...
        self.conv1 = init_fn( nn.Conv2d(in_channels, 32, [8,8], 4) )
        self.conv2 = init_fn( nn.Conv2d(32, 64, [4,4], 2) )
        self.conv3 = init_fn( nn.Conv2d(64, self._embedding_dim, [3,3], 1) )
        self.flatten = Flatten()
        self.fc_1 = init_fn( nn.Linear(self.fc_channels, 512) )
    ...
    def get_local_features(self, inputs):
        inputs = self.local_encoder(inputs)
        # VQ layer forward
        ...
        outputs = quantized.permute(0, 3, 1, 2).contiguous()
        return outputs, loss, perplexity, encodings

    def forward(self, inputs):

        outputs, _, _, _ = self.get_local_features(inputs)

        features = self.global_encoder(outputs)
        return features
