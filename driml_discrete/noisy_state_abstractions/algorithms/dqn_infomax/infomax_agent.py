...
from noisy_state_abstractions.algorithms.dqn_infomax.losses import InfoNCE_action_loss, VQ_InfoNCE_action_loss
...
class InfoMaxAgent(DQNAgent):
    ...
    def update(self, target, memory):
        ...
        if self.lambda_LL >0 or self.lambda_LG >0 or self.lambda_GL >0 or self.lambda_GG > 0:
            ...
            # print("update in infomax agent")
            if self.encoder_name.startswith('VQ'):
                dict_nce = VQ_InfoNCE_action_loss(self.encoder, s_t, a_t, r_t, s_tpk, self.device, {'data_aug': False,
                                                                                                 'ema_moco': False,
                ...
                lambda_phi = 1e-3
                # increase lambda_rl when adding vq_loss
                self.lambda_rl = 10.0
                vq_loss = lambda_phi * dict_nce['vq_loss']
            else:
        ...
        if self.encoder_name.startswith('VQ'):
            vq_loss_value = vq_loss.cpu().detach()
        else:
            vq_loss_value = 0.
        return {'loss_rl':loss_rl.cpu().detach(),'loss_infomax':loss_nce.cpu().detach(), 'vq_loss':vq_loss_value}