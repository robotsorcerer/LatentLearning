...
def VQ_InfoNCE_action_loss(model, s_t, a_t, r_t, s_t_p_1, device, args, s_t_p_k=None, target=None):
    score_fn = nce_scores_log_softmax
    ...
    local_encoder = model.get_local_features  # Local encoder
    global_encoder = model.global_encoder  # Global encoder
    action_encoder = model.action_encoder

    # Extract features phi(s)
    s_t_local, loss1, _, _ = local_encoder(s_t.float())
    # print("s_t_local", s_t_local.shape)
    ...
    if args['ema_moco']:
        ...
        s_t_p_1_local, loss2, _, _ = model.get_local_features(s_t_p_1.float())
    if args['lambda_GL'] != 0 or args['lambda_GG'] != 0:
        s_t_global = global_encoder(s_t_local)
    if args['lambda_LG'] != 0 or args['lambda_GG'] != 0:
        if args['ema_moco']:
    ...
    a_t_global = action_encoder(a_t)
    a_t_local = a_t_global
    a_t_local = a_t_local.unsqueeze(-1).unsqueeze(-1).repeat(1, 1, s_t_local.shape[2], s_t_local.shape[
        3])  # repeat HxW times to be compatible with local concats
